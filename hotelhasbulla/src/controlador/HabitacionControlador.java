...
public class HabitacionControlador implements HabitacionRepository {
    ...
    public void addHabitacion(Habitacion habitacion) {
        try {
            PreparedStatement statement = connection.prepareStatement(
                "INSERT INTO habitacion (numero_habitacion, tipo, descripcion, precio, disponibilidad, limpieza) VALUES (?, ?, ?, ?, ?, ?)"
            );
    ...
    public Habitacion getHabitacionByNumero(int numero_habitacion) {
        ...
        try {
            PreparedStatement statement = connection.prepareStatement("SELECT * FROM habitacion WHERE numero_habitacion = ?");
    ...
    public void updateHabitacion(Habitacion habitacion) {
        try {
            PreparedStatement statement = connection.prepareStatement(
                "UPDATE habitacion SET tipo = ?, descripcion = ?, precio = ?, disponibilidad = ?, limpieza = ? WHERE numero_habitacion = ?"
            );
    ...
    public void deleteHabitacion(int numero_habitacion) {
        try {
            PreparedStatement statement = connection.prepareStatement("DELETE FROM habitacion WHERE numero_habitacion = ?");
            statement.setInt(1, numero_habitacion);
